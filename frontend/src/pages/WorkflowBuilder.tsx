  ...
  return (
    <div className="h-full flex flex-col space-y-4">
      <div className="flex flex-col xl:flex-row justify-between items-start gap-4">
        ...
        <div className="flex flex-col sm:flex-row w-full xl:w-auto gap-3 items-stretch sm:items-center">
          <input
            type="text"
            placeholder="Workflow run input"
            className="border border-slate-200 rounded-lg px-3 py-2 text-sm disabled:bg-slate-100 disabled:text-slate-400 w-full sm:min-w-[320px] sm:w-auto"
            value={runInput}
            disabled={isLoading || !!error || isRunning}
            onChange={e => setRunInput(e.target.value)}
          />
          <div className="flex gap-2 w-full sm:w-auto">
            <button disabled={!canSave} onClick={handleSave} className="flex-1 sm:flex-none flex items-center justify-center space-x-2 bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed">
              <Save size={18} />
              <span>{isSaving ? UI_LABELS.SAVING : UI_LABELS.SAVE}</span>
            </button>
            <button disabled={!canRun} onClick={handleRun} className="flex-1 sm:flex-none flex items-center justify-center space-x-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed">
              <Play size={18} />
      ...
      <div className="flex-grow min-h-[500px] lg:flex-1 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
        {isLoading && (
